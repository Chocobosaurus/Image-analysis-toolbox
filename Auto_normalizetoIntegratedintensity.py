...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 10 13:39:06 2024
...
def process_image(input_path, output_txt):
    ...
    intensity_ratio = calculate_intensity_ratio(imageA, imageB)
    print("Intensity Ratio:", intensity_ratio)
    
    # Find the corresponding value to 10000 in the histogram distribution
    target_bin = 10000
    target_intensity = target_bin * intensity_ratio
    print("Target intensity value found:", target_intensity)

    # Write the corresponding value to the output text file
    with open(output_txt, 'a') as file:
        parent_dir_name = os.path.basename(os.path.dirname(input_path))
        file.write(f"{parent_dir_name}_{os.path.basename(input_path)}, {target_intensity}\n")
